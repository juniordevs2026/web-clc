...
app.post('/api/courses', requireAdmin, asyncRoute(async (req, res) => {
  const input = z.object({ namaPelajaran: z.string().min(2), deskripsi: z.string().min(5), guruId: z.number().int().positive(), kapasitas: z.number().int().positive().max(3).default(3), kelas: z.string().min(1).max(40) }).parse(req.body);
  const classExists = await pool.query('SELECT 1 FROM kelas WHERE nama_kelas = $1', [input.kelas]);
